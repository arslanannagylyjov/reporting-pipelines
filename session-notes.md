...
## 2026-09-01 — `supplier_orders_pending` pipeline (Cansun/Karacan "Siparişte" report)

New sync job — the "not yet an order" counterpart to `orders_in_transit`. `~/reporting-scripts/refresh_supplier_orders_pending.py` on athena syncs `reporting.supplier_orders_pending` (PK `(Firma, ID)`, 2,194 rows first run, all `Firma = 'Cansun Sipariste'` — Karacan has none pending, Almer deferred) from ERP view `cansun.aa_rapor_sipariste_hepsi`. Cron `20 3 * * *` (03:20, in the 03:15 → 03:45 gap between `cheque_bond_maturity` and `stock_details`).

- **Full replace via `DELETE FROM` + chunked `INSERT` (`CHUNK_SIZE = 5000`), not an upsert.** The source view filters on `EvrakNoSiparis IS NULL`, so a pending request vanishes from the view the moment it's converted to a firm order and never reappears under the same `ID` — an upsert would strand every converted line forever as still-pending (the `sales_snapshot` staleness pattern). Wipe-and-reload sidesteps it: no prune step, no staging table. **`DELETE`, not `TRUNCATE`:** `reporting_writer` has `SELECT, INSERT, UPDATE, DELETE` with no `DROP` on this table (`SHOW GRANTS` up front), same substitution as `warehouse_stock` / `cek_senet_portfoy`.
- **No pre-flight duplicate check** — live source is 2,194 rows / 2,194 distinct `(Firma, ID)`, and a full wipe-and-reload can't collapse rows the way a bad-key upsert would (same call as `warehouse_stock` / `customer_last_price`).
- **Column map, not `SELECT *`:** the ERP view aliases three columns with a space in the name (`` `Doviz Fiyat` ``, `` `Doviz Tutar` ``, `` `Evrak Tutari` ``) → `DovizFiyat` / `DovizTutar` / `EvrakTutari` on `reporting-db`, verified against `SHOW COLUMNS` on both sides.
- **Telegram — failure only, in the 07:00 digest from day one.** No per-run success ping was ever added (learning from `orders_in_transit` / `warehouse_stock`, which had theirs removed 2026-08-28). Thin `try`/`except` around `run_job('supplier_orders_pending', main)` fires `[supplier_orders_pending] FAILED: …` on any crash. Added to `monitored_jobs.yml` with `expected_time: "03:20"` → **8th registry job**, digest now covers eight.
- **Metabase:** native question `03_Ithalat_SiparişteÜrünListesi` (card 105, `SELECT * FROM supplier_orders_pending`, table viz) created directly in `3. İthalat/İhracat` (coll 10). No dashboard, no group override, no permission-graph change — inherits folder-10 cascade (`Director`, `Manager`, `Sales/Purchase`, `Satış`). No cost column, so no boss-only exposure decision. Verified via `GET /api/card/105`: `collection_id: 10`, `query_type: native`, not archived, 16 result columns matching the table.
- **First run (2026-09-01 23:48:37):** 2,194 rows fetched, 2,194 inserted, `status=ok` / `0.31s` in `job_runs.csv`, no Telegram. Source/dest counts match exactly.

**Docs:** all six per the `docs/adding-a-new-report.md` Step 13 checklist — `docs/tables.md` (new section), `docs/monitoring.md` (8-job registry + `monitored_jobs.yml` YAML block, which was still missing `orders_in_transit`/`warehouse_stock`, + new per-job section), `docs/metabase-permissions.md` (new dated section for card 105), `docs/server-architecture.md` and `README.md` (both had been stale since `stock_details` — caught up here for `orders_in_transit`, `warehouse_stock` **and** `supplier_orders_pending`: cron block, job counts, nightly-batch order, digest list, Tables summary, grants note), and this entry. Script lives only on athena (per convention). Also corrected `docs/monitoring.md`'s "13 distinct `job_name`s as of 2026-08-28" — that count had included the CSV header row; the real figure was 12, now 13 with `supplier_orders_pending`.

## 2026-09-02 — `product_min_max_stock` pipeline (min/max stocking report)

New nightly sync (cron `25 3 * * *`): `reporting.product_min_max_stock` from ERP view `cansun.aa_product_min_max_stock` — per-product 6-month min/max/total demand plus current on-hand quantity, an aggregate snapshot (one row per `ProductId`, 1,078 rows). Feeds a slow-mover / min-max stocking report.

- **Spec said `TRUNCATE` + full reload in a single transaction. That's not possible as written** — MySQL `TRUNCATE` is DDL, it forces an implicit commit and can't be inside a transaction, and separately `reporting_writer` has no `DROP` grant here (`TRUNCATE` needs it). Used `DELETE FROM` + chunked `INSERT` with `autocommit = False` and **one commit at the end** instead — this is what actually delivers the spec's stated intent ("a mid-run failure never leaves the table empty for readers"): a failure rolls back and Metabase keeps seeing the previous 1,078 rows. Same `TRUNCATE`→`DELETE` substitution already used by `cek_senet_portfoy` / `warehouse_stock`; the single-commit-at-end is stricter than `customer_last_price` (which commits per batch).
- **Pre-write sanity check** (before the `DELETE`): abort — leaving the table untouched — if the ERP pull is empty, or if row count is < 50% of the previous successful run's count in `job_runs.csv`. A reload has no prune-step equivalent, so an upstream view silently breaking to empty would otherwise wipe the report table every night. Adapted from `cek_senet_portfoy`'s pre-flight check (upsert → reload). Abort raises `RuntimeError` → `job_runs.csv` `fail` + `[product_min_max_stock] FAILED: …` Telegram.
- **Explicit column list, not the spec's literal `SELECT *`** — source and dest column names/order are identical (verified `SHOW COLUMNS` both sides), but the explicit list is the standing convention and guards against a future source reorder. `QuantityInStorage` narrows `bigint`→`int` (safe, small counts).
- **Runtime ~11s, essentially all ERP-side** — standalone timing of just `SELECT * FROM cansun.aa_product_min_max_stock` was 11.8–12.1s (it aggregates over an upstream monthly view); the local write is sub-second. At 03:25 it clears by ~03:25:12, before `vault_movements_daily` (03:30). Flagged to reconfirm after real cron cycles since the ERP time is the variable.
- **Registry:** added to `monitored_jobs.yml` at `expected_time: "03:25"` — the **9th** registry job (spec said "8th" — off by one; `supplier_orders_pending` took the 8th slot on 2026-09-01). 14th distinct `job_name` in `job_runs.csv`. Failure-only Telegram, no success ping.
- **Metabase:** native question `01_Üretim_MinMaxStok` (card 106, `SELECT * FROM product_min_max_stock`, table viz) created directly in `5. Envanter Yönetimi` (coll 11) — **its first content, previously empty**. No dashboard, no group override, no permission-graph change (graph still rev 16). Folder 5 is the bottom of the cascade, so the question is visible to **every** group including `Other` — intended for a stocking report, no cost column.
- **Verified:** 3 manual runs (all `ok`, ~11s, 1,078 rows); drop-check exercised with a real previous count (1,078→1,078, no abort) and with a synthetic 99,999 previous count (aborted **before** `DELETE`, table stayed at 1,078, `fail` logged, Telegram fired — confirms the alert path end-to-end; that one test message went to the real `@cansun_reporting_bot` channel). `report_job_status.py` dry run lists all 9 registry jobs `✅`, no false "DID NOT RUN". Card 106 executes → 1,078 rows / 7 cols.

**Docs:** all six per the Step 13 checklist — `docs/tables.md`, `docs/monitoring.md` (spec's STEP D named only these two; extended to the full six per the standing playbook), `docs/metabase-permissions.md`, `docs/server-architecture.md`, `README.md`, and this entry. No GRANT SQL written or run (Arslan confirmed grants complete). Script on athena only.
...
## 2026-09-24 — Dashboard 3: tab-specific filters, new Coğrafya cards, and an inverted Firma filter fixed on 17 cards

Six Metabase-only tasks, all through the REST API (API key read from athena's `.env` over SSH, held in memory, never written anywhere). No table, sync script, cron, GRANT or athena change; nothing in the repo besides these docs.

**Headline — the `Firma` filter returned everything EXCEPT the selection on 17 cards (42–47, 49–54, 110–114).** Root cause: these cards wrote the field filter as `[[AND Firma = {{firma}}]]`. A Field Filter tag renders as the *whole* condition, so Metabase sent `Firma = (sales_snapshot.Firma = 'Almer')` — true for every row except Almer's (the string coerces to 0, the predicate is 0/1). No error is raised and an empty Firma is correct, so it went unnoticed. It surfaced on card 54, where a table alias (`FROM sales_snapshot s`) made the filter *error* — fixing the alias exposed the inversion (Almer showed the Cansun+Karacan total, and the three per-Firma totals summed to exactly 2× the empty total). Fix on every card: `[[AND {{firma}}]]`, query only — tags, display, visualization settings, collection and mappings untouched.
- **Card 54** was fixed first (alias removed + filter clause). The other **16** came from an instance-wide audit (60 non-archived native cards, 40 field-filter tags — no other misuse found; no `IN`/`LIKE`/aliased-table cases). That included **card 42 `Toplam Ciro`**, an orphan not on any dashboard, changed as part of the same fix. The text/number tags used as `col = {{tag}}` (`year`/`month` on cards 80–88 and 99–102, `stok_durumu` on card 106) are correct and were left alone.
- **Verified against ground truth — 240/240.** 17 cards × Tarih (`thisyear`, `thismonth`; n/a for 43/44/47/54, which have no Tarih tag) × 8 Firma selections (empty, each single, each pair, all three). Ground truth = the card's own new SQL with each tag replaced by a literal condition, run via `POST /api/dataset`. Checked: (a) rows identical, same order; (b) SQL `SUM` around the card's rendered SQL equals the ground truth's (not summing card output — the grid is capped at 2,000 rows); (c) single-Firma totals sum to the empty total, pairs equal the sum of their members. (c) is not applicable to card 46 (an average) and card 112 (top 15); both still passed (a)/(b).
- **Consequence to remember: any Firma-filtered number taken from these cards before 2026-09-24 was inverted.** Results with Firma empty were always correct.

**Tab "Ürünler & Müşteriler" (tab id 5).** Card 54 `Tedarikçiye Göre Aylık Satış (Sabit)` moved to the top, full width; below it card 50 and card 51, full width, 8 rows tall each. **Card 50 `En Çok Satan Ürünler`** is now a table grouped by `StokKodu` + `KompleAdi` (the destination column name — the ERP source calls it `Komple Adi`), no `LIMIT`, `ORDER BY ciro DESC`, `ciro` with thousands separators; Metabase caps the displayed grid at **2,000 rows**, so the tail is not shown. **Card 51 `En İyi Müşteriler`** gained `HesapKodu` as its first column (grouped by `HesapKodu, CariAdi`). The tab has its **own filters `tarih_urunler` / `firma_urunler`** (new dashboard parameters, default `thismonth`; card 54 maps to Firma only, as before).

**Tab "Coğrafya & Kanal" (tab id 6).** Own filters **`tarih_cografya` / `firma_cografya`** (same shape, default `thismonth`); the original shared `Tarih`/`Firma` (default `thisyear`) now feed only `Genel Bakış`, so changing filters on one tab no longer affects another. **Card 52 `Bölgesel Satış`** is now Yurtici only (`SatisTipi = 'Yurtici'`). **Five new questions, created directly in `_Hesaplama Kaynağı` (coll 6)**, native SQL on `sales_snapshot`, no table alias, same `SUM(TutarTL) AS ciro`: **110** `Yurtiçi Satış: İstanbul / Diğer` (donut; the only spelling in the data is `ISTANBUL`, the CASE also accepts `İSTANBUL`; blank/NULL `Il` falls under Diğer), **111** `İhracat Satışları (Ülkeye Göre)` (horizontal bar, all countries, `graph.max_categories_enabled: false` so the tail isn't grouped into "Other"), **112** `Yurtiçi Satış: İl Bazında İlk 15`, **113** `Plasiyer Bazında Kanal Dağılımı` (stacked Yurtici vs Ihracat per salesperson), **114** `İhracat: Döviz Cinsine Göre Satış` (USD/EUR, TL and original-currency amounts). Layout: 52 | 53 on top, 110 | 111 below (15 rows tall for the ~50-country chart), then 112 | 113, then 114.

**Card 57 `02_Ihracat_SonAlışTedarikçiFiyat`** (coll 10, table `supplier_last_purchase`): `Firma` is now a 3-value static dropdown (`Almer`, `Cansun`, `Karacan`, exact spellings from `SELECT DISTINCT`). The dropdown lives on the card-level `parameters` entry (`values_query_type: list`, `values_source_type: static-list`), **not** on the template tag — SQL and tags unchanged, and it *is* settable through `PUT /api/card/:id` with `{parameters}` (an earlier assumption that it needed the UI was wrong). Counts (SQL `COUNT`, grid is capped): empty 22,933 = 3,435 + 16,334 + 3,164. Card is on no dashboard.
