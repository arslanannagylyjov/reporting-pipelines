...
25 3 * * *      /usr/bin/python3 /home/arslan/reporting-scripts/refresh_product_min_max_stock.py >> /home/arslan/reporting-scripts/refresh_product_min_max_stock.log 2>&1
...
Fifteen crontab lines on athena as of 2026-09-02 — 13 sync-script jobs plus two non-sync jobs (`bump_filter_defaults.py` monthly, `report_job_status.py` the 07:00 digest) — grown from the original two, each sync job added incrementally alongside a new synced table; see `docs/monitoring.md` and `session-notes.md` for the history and reasoning behind each addition's cron time. Grouped by cadence:

- **Nightly batch (02:30–03:45):** `sales_snapshot` → `customer_last_price` → `supplier_last_purchase` → `orders_in_transit` → `warehouse_stock` → `cheque_bond_maturity` → `supplier_orders_pending` → `product_min_max_stock` → `vault_movements_daily` → `stock_details`, each offset 5–15 minutes past the previous based on that job's own observed run duration (most finish in well under a minute; exceptions: `stock_details` ~7s for 40k rows, and `product_min_max_stock` ~11s — almost all of it the ERP aggregate view query, not the write). `bump_filter_defaults.py` shares the 03:30 slot but only actually fires on the 1st of the month.
- **07:00 daily digest:** `report_job_status.py` sends one Telegram summary covering the nine jobs registered in `monitored_jobs.yml` (`sales_snapshot`, `customer_last_price`, `supplier_last_purchase`, `orders_in_transit`, `warehouse_stock`, `cheque_bond_maturity`, `supplier_orders_pending`, `product_min_max_stock`, `stock_details`) — see `docs/monitoring.md` for why every other job below is deliberately excluded from that registry (their schedules don't fit a "ran once overnight" daily-freshness check) and instead use failure-only or per-run Telegram alerting of their own.
...
| `product_min_max_stock` | 1,078 (2026-09-02) | Per-product 6-month min/max/total demand + on-hand qty, aggregate snapshot (`DELETE`+`INSERT` full replace, single transaction, pre-write row-drop guard) |
