...
### Monthly tiles converted to self-rolling — hardcoded months removed (2026-09-07)

**Superseded the "explicit `(year, month)` pairs" design above.** Those 7 cards (72–78) had no automation and went stale: on 2026-09-07 the tab still showed Ağustos 2026 – Şubat 2027 while `MAX(VadeTarihi)` had moved to 2027-03-19 and the top chart (card 79) had already rolled to Eylül 2026 – Mart 2027. `bump_filter_defaults.py` only ever bumped cards 80–88, never these.

**Fix:** each card's `date_series` anchor is now `DATE(DATE_FORMAT(CURDATE(), '%Y-%m-01') + INTERVAL N MONTH)` for a fixed offset `N` (card 72 → N=0 … card 78 → N=6), so the 7 tiles always span the current month through +6 with no maintenance. Cards renamed from absolute months to `Takip — Bu Ay` / `Takip — Gelecek Ay` / `Takip — 2 Ay Sonra` … `Takip — 6 Ay Sonra`. Added a leading `Ay` column (`CONCAT(ELT(MONTH(...), 'Ocak',…,'Aralık'), ' ', YEAR(...))`) to carry the absolute month now that the title doesn't; **dropped the `HaftaGunu` raw-weekday column** to keep the half-width (`size_x: 12`) tile at 4 columns with no horizontal scroll (`TatilDurumu` still flags weekends via the same `WEEKDAY(...) IN (5,6)` test). The `DATE(...)` wrapper matters — without it the recursive CTE's anchor column is a string type and Metabase drops the `D/M/YYYY` formatting on `Vade Tarihi`. `table.column_formatting` and `column_settings` preserved on all 7. Only cards 72–78's own `dataset_query` / `visualization_settings` / `name` were `PUT`; `/api/dashboard/3/cards` untouched (dashboard still 41 dashcards, tab `Çek Takip` still 8). Details + the Ay-filter false alarm from the same report in `docs/monitoring.md` ("Dashboard 3 …staleness — diagnosed 2026-09-07"). Template above is updated by this: new tiles should be offset-from-`CURDATE()`, not `(year, month)` literals.
...
New native question against `product_min_max_stock` (table visualization) with an explicit column list rather than `SELECT *`, each column aliased to a Turkish display name in the card's SQL: `ProductId` → `Ürün Kodu`, `ProductName` → `Ürün Adı`, `MinMonthlyQty` → `Aylık Min. Satış`, `MaxMonthlyQty` → `Aylık Maks. Satış`, `Total6mQty` → `6 Aylık Toplam Satış`, `AvgPerMonthQty` → `Aylık Ortalama Satış`, `QuantityInStorage` → `Depo Stok Miktarı`. The aliases are Metabase display-layer only — the `reporting-db` table and column names remain English. Backs the `product_min_max_stock` sync pipeline added the same day (see `docs/tables.md` / `docs/monitoring.md`). Created directly in collection 11 (`5. Envanter Yönetimi`), which was **previously empty** — this is its first content. **No permission-graph change, no new group, no GRANTs, no collection move.**
