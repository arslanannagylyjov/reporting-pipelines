...
**Metabase:** this table backs four native questions in the `_Hesaplama Kaynağı` collection (id 6), all on dashboard 3's `Kasa Durumu` tab, all reading `reporting-db` (database id 3):

- **`Kasa Hareketleri - TL`** (card 93), **`- EUR`** (card 94), **`- USD`** (card 95) — one per vault, filtered by `KasaKodu` (`100-00` / `100-01` / `100-02`), each `SELECT KasaAciklamasi, IslemTarihi, Aciklama, tutar_giren, tutar_cikan[, DovizKodu] FROM vault_movements WHERE KasaKodu = '…' AND {{date_range}} ORDER BY IslemTarihi DESC`. The TL card omits `DovizKodu` (its rows are mostly blank in the source); EUR/USD include it. `{{date_range}}` is a field filter on `IslemTarihi`, default `past7days~`. Grid rows 24 / 33 / 42, full width (`size_x=24`, `size_y=9`).
- **`Kasa Hareketleri - Tümü`** (card 108, added 2026-09-06) — `UNION ALL` of the three cards' exact underlying logic into one query, with a hardcoded `'TRY' AS DovizKodu` literal added to the TL branch (`100-00`) so every row carries a currency code (same literal-column pattern as `aa_bank_movements`' `Firma`); EUR/USD branches keep their real `DovizKodu` column. Same `{{date_range}}` field filter, used once per branch. Carries the same currency-coded **whole-row conditional formatting** as card 107 (`TRY` → `#F9F9FA`, `USD` → `#88BF4D` green, `EUR` → `#F9D45C` yellow; distinct values confirmed live against the unioned `past7days~` result — a small number of blank-`DovizKodu` rows exist in the `100-02` branch and render uncolored). Placed at grid row 51 (directly below the three per-vault cards), full width. **Currently a test/comparison card, not a replacement** — the three per-vault cards are untouched (row counts on the 2026-09-06 fresh load: 19 + 6 + 14 = 39, exactly matching the combined card). Verified via `GET /api/dashboard/3`: tab 11 went 10 → 11 dashcards, all other cards' positions and all other tabs unchanged.
...
Bank-account movement lines (deposits, withdrawals, transfers, cheque/senet settlements) across all three companies — a **transaction log, one row per movement line per company**, full history with no date filter. 15,450 rows as of the 2026-09-04 initial sync (Cansun 10,752 / Almer 4,006 / Karacan 692). Sourced from the pre-built ERP view `cansun.aa_bank_movements` on Natra via `metabase_ro`; despite the `cansun` schema prefix the view is a **3-way `UNION ALL`** of the Cansun, Almer and Karacan bank-movement tables, keyed on `Firma`. The `SELECT` is sub-second — this job's ~1.7s runtime is almost entirely the local write.
...
| ID | bigint | NO | PRI |
| EvrakNo | varchar(50) | YES | |
| BankaKodu | varchar(30) | YES | |
| BankaAciklamasi | varchar(200) | YES | |
| Aciklama | varchar(255) | YES | |
| HesapKodu | varchar(30) | YES | |
| HesapAciklamasi | varchar(200) | YES | |
| Borc | decimal(23,6) | YES | |
| Alacak | decimal(23,6) | YES | |
| BorcYerel | decimal(23,6) | YES | |
| AlacakYerel | decimal(23,6) | YES | |
| IslemTipi | varchar(50) | YES | |
| DovizKodu | varchar(10) | YES | |
| DovizKuru | decimal(23,6) | YES | |
| IslemTarihi | datetime | YES | |
| VadeTarihi | datetime | YES | |
| Seri | varchar(20) | YES | |

Composite primary key on `(Firma, ID)` — `ID` is only unique *within* a company, so `Firma` is part of the key (same shape as `supplier_orders_pending` / `orders_in_transit`). Column names and order are identical on the ERP view and the destination table; the sync uses an explicit column list rather than `SELECT *` all the same. Every destination column is the same type or wider than the source: source `Firma` is `varchar(7)`, `ID` is `int`, the money columns and `DovizKuru` are `double`, and `IslemTarihi` / `VadeTarihi` are `DATE` (widened to `DATETIME` here) — all safe widenings.

**Load pattern — `DELETE` + chunked `INSERT` (`CHUNK_SIZE = 5000`) in a single transaction, one commit at the end.** This is a full reload of the entire history every run — there is **no prune step** (nothing goes stale: a movement line that's edited upstream keeps its `(Firma, ID)` and is simply re-loaded with its new values; the wipe-and-reload picks that up for free). Two deliberate deviations from the spec's literal `TRUNCATE`, identical to `product_min_max_stock`:

- **`DELETE FROM`, not `TRUNCATE`.** `reporting_writer` has `SELECT, INSERT, UPDATE, DELETE` but no `DROP` on this table (`SHOW GRANTS`), and MySQL `TRUNCATE` requires `DROP` — same substitution as `product_min_max_stock` / `cek_senet_portfoy` / `warehouse_stock`. Separately, `TRUNCATE` is DDL: it forces an implicit commit and **cannot be inside a transaction**, so the spec's own requirement ("single transaction so a mid-run failure never leaves the table empty for readers") is only achievable with `DELETE`.
- **Single transaction, single commit.** The `DELETE` and every `INSERT` batch run in one uncommitted transaction (`autocommit = False`); a mid-run failure rolls back and a reader keeps seeing the previous 15,450 rows until the new set commits atomically.

**Pre-write sanity check (before the `DELETE`):** the job aborts — leaving the table untouched — if the ERP pull returns 0 rows, or if the row count is under 50% of the previous successful run's count recorded in `job_runs.csv`. Because this is a full reload with no prune-step equivalent, this guard is the only thing standing between "the 3-way `UNION ALL` view broke upstream" and "the whole history table silently emptied." On the first run there is no previous count, so only the empty check applies.

Refreshed nightly at **03:35** by `refresh_bank_movements.py` — see `docs/monitoring.md` for the schedule reasoning and the failure-only Telegram alerting (prefix `[bank_movements]`).

**Metabase (2026-09-05):** backs the native question **`Banka Hareketleri`** (card 107) in the `_Hesaplama Kaynağı` collection (id 6, nested under `1. Yöneticiler`) — per the standing rule, supporting cards for dashboard components live there, never top-level. SQL: `SELECT Firma, DATE_FORMAT(IslemTarihi, '%d.%m.%Y') AS IslemTarihi, BankaAciklamasi, HesapAciklamasi, DovizKodu, Borç (BorcYerel for TRY else Borc), Alacak (AlacakYerel for TRY else Alacak), DovizKuru FROM bank_movements WHERE IslemTarihi >= CURDATE() - INTERVAL 7 DAY ORDER BY bank_movements.IslemTarihi DESC` — a **7-day rolling window** (81 rows on the 2026-09-05 initial placement), reading from `reporting-db` (`metabase_reporting_db`, database id 3), the same connection as card 106 and every other reporting-table question, not the ERP connection. The `Borç`/`Alacak` columns pick the local-currency (`*Yerel`) figure for `TRY` rows and the original-currency figure otherwise, so both columns always read in the transaction's own currency — this flip is possible only because `bank_movements` carries both the original and local-currency figure per row (see the column table above); `Borç`/`Alacak` are Turkish display aliases in the card's SQL only, not columns on the underlying table.

`IslemTarihi` is reformatted to `DD.MM.YYYY` (e.g. `04.09.2026`) via `DATE_FORMAT`, dropping the `00:00` time suffix Metabase otherwise renders on a `DATE`-typed column with no real time component — display-only, the underlying `bank_movements.IslemTarihi` column and the 7-day `WHERE` filter are untouched. **Non-obvious side effect fixed in the same edit:** aliasing the formatted expression back onto the name `IslemTarihi` made MySQL's original `ORDER BY IslemTarihi DESC` resolve to that new string alias instead of the source date column — since `DD.MM.YYYY` strings don't sort chronologically (`"31.08.2026"` sorts before `"04.09.2026"` alphabetically), rows came back out of date order. Fixed by qualifying the `ORDER BY` as `bank_movements.IslemTarihi DESC`, which forces resolution to the real table column regardless of the SELECT-list alias; verified live (row order back to strict most-recent-first) before saving.

Card 107 also carries **conditional formatting** on `DovizKodu`, confirmed against the live distinct values in the 7-day window (`EUR` / `TRY` / `USD`, matching the expected three) rather than assumed: `TRY` → `#F9F9FA` (near-white/neutral), `USD` → `#88BF4D` (green, same swatch as card 106's `Yeterli`), `EUR` → `#F9D45C` (yellow). Each rule has **"Highlight the whole row"** enabled (the toggle beneath the color picker in the rule editor) — cell backgrounds visibly reach every column, not only `DovizKodu`, confirmed both on the standalone question and on the dashboard tile by inspecting each row's per-column background style. Configured in the Metabase UI (rule editor, not derivable from the SQL) and saved via **Replace original question** — same flow as card 106's conditional formatting (see `docs/monitoring.md`'s Metabase conditional-formatting notes). If a fourth currency code ever appears in the source it will render uncolored until a rule is added, same caveat as card 106's two-value `Stok Durumu` formatting.

**Column widths** narrowed in the same settings pass: `BankaAciklamasi` 260px → 160px, `HesapAciklamasi` 375px → 180px (`visualization_settings["table.column_widths"]`, an array positional to the column order — `[null, null, 160, 180, …]`). Together with the `IslemTarihi` date shortening this brought the table's total rendered width from ~1274px to ~937px, comfortably inside a standard dashboard tile with no horizontal scrollbar; `IslemTarihi` itself didn't need narrowing. Overflow text truncates with a CSS ellipsis — the full value stays in the underlying data and the page's accessibility tree (confirmed: `scrollWidth > clientWidth` on a truncated cell, full string still present in `innerText`), so nothing is data-lossy. **Caveat found during verification, worth knowing before promising it to anyone:** this Metabase table renderer has **no hover tooltip** revealing the full truncated value (no `title` attribute is set, and no tooltip portal appears on hover) — clicking a truncated cell opens the standard column filter popover ("Is this" / "Is not this") instead, which also doesn't surface the full text. The only ways to see a truncated value in full today are downloading the results or widening the column again in the editor.

Placed on the **`Cansun Satış Genel Bakış`** dashboard (id 3), **`Kasa Durumu`** tab (id 11), inserted between `Portföy Çek/Senet Listesi` (ends at grid row 15) and `Kasa Hareketleri - TL` (pushed from row 15 to row 24) — full width (`size_x=24`), `size_y=9`, matching the other full-width tables on that tab. Only `Kasa Hareketleri - TL/EUR/USD` shifted down by 9 rows to make room; every other card on this tab and every other tab's cards are untouched (confirmed via `GET /api/dashboard/3`: 40 dashcards total, was 39, all six tabs' per-tab card counts otherwise unchanged). No dashboard-level filter (`Tarih`/`Firma`/`Yıl`/`Ay`) is mapped to this card (`parameter_mappings: []`, matching every other card on the `Kasa Durumu` tab) — its date window is the fixed 7-day `WHERE` clause baked into the SQL, independent of the dashboard's `Tarih` filter, same fixed-window pattern as the `Kasa Durumu` tab's other cards.
